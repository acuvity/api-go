...
Type: [`egressdefaultpolicy`](#egressdefaultpolicy)
...
Type: [`[]ingresslistener`](#ingresslistener)
...
Represents a diagnostic message for a gateway instance.
...
Represents the default egress policy configuration of an application component.
...
Type: [`[]egresspolicyacl`](#egresspolicyacl)
...
Type: [`[]egresspolicyrule`](#egresspolicyrule)

Rules that define how egress traffic is handled.

### EgressPolicyACL

Represents an ACL for an egress policy of an application component.
...
Represents a rule for an egress policy of an application component.
...
Type: `enum(NotApplicable | Allow | Deny)`
...
Represents an ACL for the ingress of an application component.
...
```json
{
  ...
  "action": "Allow",
  "description": "Allows access from loadbalancer IPs.",
  "disabled": false,
  "name": "allow loadbalancer access",
  "port": 8443
}
```
...
Represents a listener for the ingress of an application component.
...
##### `port` [`required`,`min_value=1.000000`,`max_value=65535.000000`]
...
Type: [`ingressproxyconfig`](#ingressproxyconfig)
...
Type: [`[]ingresspolicyrule`](#ingresspolicyrule)

The list of rules for the ingress policy.

### IngressPolicyRule

Represents a rule for an ingress policy of an application component.
...
Represents the proxy configuration for an ingress listener of an application
component.
...
Type: `enum(Preserve | AutoDetect | Enabled | Disabled)`
...
Type: `enum(AutoDetect | Enabled | Disabled)`
