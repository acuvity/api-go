...
Represents the license for the organization.
...
{
  "active": false,
  "end": "2025-03-22T14:35:00.123456789Z",
  "seats": 1,
  "start": "2025-03-22T14:35:00.123456789Z",
  "type": "Evaluation"
}
```
...
##### `seats` [`required`,`min_value=1.000000`]
...
Type: `enum(Production | Evaluation)`
...
##### `agentEnforcementPolicyComputed` [`autogenerated`,`read_only`]
...
##### `scanRunningProcesses` [`autogenerated`,`read_only`]
...
Type: [`[]agentdiscoveredmcp`](#agentdiscoveredmcp)
...
##### `plugins` [`autogenerated`,`read_only`]
...
Type: [`[]agentdiscoveredskill`](#agentdiscoveredskill)

The list of configured skills for the application.

##### `type`

Type: `enum(IDE | Browser | Native)`
...
Represents an MCP server that the agent discovered.
...
### AgentDiscoveredSkill

Represents a skill that the agent discovered.
...
Agent Enforcement Policies define the local action an agent takes
...
```json
{
  ...
  "criteria": "[
  {
    \"key\": \"AIDomain\",
    \"operator\": \"Any\",
    \"values\": [\"chat.openai.com\", \"claude.ai\"]
  },
  {
    \"key\": \"ProcessName\",
    \"operator\": \"Any\",
  ...
  "message": "This AI tool is not permitted by corporate policy.",
...
##### `GET /agentenforcementpolicies`

List all available agent enforcement policies.

Parameters:

- `q` (`string`): This is an example.

##### `POST /agentenforcementpolicies`

Creates a new agent enforcement policy.

##### `DELETE /agentenforcementpolicies/:id`

Deletes the agent enforcement policy with the given ID.

##### `GET /agentenforcementpolicies/:id`

Retrieves the agent enforcement policy with the given ID.

##### `PUT /agentenforcementpolicies/:id`

Updates the agent enforcement policy with the given ID.
...
Type: `enum(BlockConnection | KillProcess | Notify)`
...
Type: [`[]agentenforcementpolicypredicate`](#agentenforcementpolicypredicate)
...
### AgentEnforcementPolicyPredicate

Represents an agent enforcement policy predicate.
...
Type: `enum(AIDomain | CommandLine | ProcessName)`

The key of the predicate.

##### `operator` [`required`]

Type: `enum(All | Any | Contains | Empty | Equals | EqualsOrGreaterThan | EqualsOrLesserThan | Like | NotAny | NotEmpty | NotEquals)`
...
AI application provides an overview of an AI-related application.
...
Creates a new AI application.

##### `DELETE /aiapps/:id`

Delete an AI application given its identifier.

##### `GET /aiapps/:id`

Retrieve an AI application given its identifier.

##### `PUT /aiapps/:id`

Update an AI application given its identifier.
...
Type: `enum(Conversational | CodingAssistant | Creative | OfflineClient | Productivity)`

The type of service this application relates to.
...
Type: [`[]aidsectionllmprovider`](#aidsectionllmprovider)

List of LLM providers the provider uses.

##### `LLMProvidersRisk`

Type: [`aidrisk`](#aidrisk)
...
Type: [`[]aidsectioncompliance`](#aidsectioncompliance)
...
Type: [`[]aidsectiondatapolicy`](#aidsectiondatapolicy)
...
Type: [`[]aidsectiondataprocessor`](#aidsectiondataprocessor)
...
Type: [`[]aidsectionplan`](#aidsectionplan)
...
### AIDSectionCompliance

AIDomain compliance information.
...
Type: `enum(Certification | Framework)`

Type of the compliance.

### AIDSectionDataPolicy

AIDomain data policy information.
...
### AIDSectionDataProcessor

AIDomain info section plan.
...
### AIDSectionLLMProvider

AIDomain info section for LLM provider.
...
Type: `enum(FirstParty | ThirdParty | Hybrid | BYOM | Unknown)`
...
Bullet-point explanation of the overall risk level, generated by the v2 policy
engine.
...
##### `DELETE /aigateways/:id`

Delete an AI Gateway given its identifier.

##### `GET /aigateways/:id`

Retrieve an AI Gateway given its identifier.

##### `PUT /aigateways/:id`

Update an AI Gateway given its identifier.

##### `GET /aigateways/:id/aigatewayclients`

Retrieves the list of AI gateway clients for this AI gateway.

##### `POST /aigateways/:id/aigatewayclients`

Creates a new AI gateway client for this AI gateway.
...
##### `oauthApplication` [`autogenerated`,`read_only`]
...
Represents OAuth clients connected to AI gateways.
...
##### `DELETE /aigatewayclients/:id`

Delete an AI Gateway Client given its identifier.

##### `GET /aigatewayclients/:id`

Retrieve an AI Gateway Client given its identifier.

##### `PUT /aigatewayclients/:id`

Update an AI Gateway Client given its identifier.
...
##### `oauthClient` [`autogenerated`,`read_only`]
...
AIGatewayConf is an API for AI gateway configurations.

#### Example

```json
{
  "revision": "2c26b46b68ffc68ff99b453c1d30413413422d706483bfa0f98a5e886266e7ae"
}
```
...
Type: [`[]aigatewayconfconnector`](#aigatewayconfconnector)
...
### AIGatewayConfConnector

The rendered connector shape consumed by the AI Gateway in an AI Gateway
configuration.
...
```json
{
  ...
  "route": "/openai-api",
  "type": "MCP",
  "upstreamURL": "https://ai.example.com/ai"
}
```
...
Type: [`[]aigatewayconfmcptool`](#aigatewayconfmcptool)
...
Type: [`[]aigatewayconftokenpool`](#aigatewayconftokenpool)
...
Type: `enum(MCP | LLM | WebhookIntegration)`
...
Type: [`webhookintegrationconf`](#webhookintegrationconf)
...
### AIGatewayConfMCPTool

The rendered tool shape consumed by AI Gateway in an AI gateway configuration.
...
The rendered token shape consumed by the AI Gateway.
...
The rendered token pool shape consumed by the AI Gateway.
...
Type: [`[]aigatewayconftoken`](#aigatewayconftoken)

The rendered tokens belonging to this pool.

### AIGatewayConnector

Represents an AI Gateway Connector attached to an AI gateway.
...
```json
{
  ...
  "type": "MCP",
  "upstreamURL": "https://ai.example.com/ai",
  "webhookIntegration": "litellm"
}
```

#### Relations

##### `GET /aigatewayconnectors`

List all available ai gateway connectors.

Parameters:

- `q` (`string`): This is an example.

##### `POST /aigatewayconnectors`

Creates a new ai gateway connector.

##### `DELETE /aigatewayconnectors/:id`

Delete an AI Gateway Connector given its identifier.

##### `GET /aigatewayconnectors/:id`

Retrieve an AI Gateway Connector given its identifier.

##### `PUT /aigatewayconnectors/:id`

Update an AI Gateway Connector given its identifier.
...
Type: [`webhookintegrationconfig`](#webhookintegrationconfig)
...
AI MCP server provides an overview of a Model Context Protocol server
...
Creates a new AI MCP server.

##### `DELETE /aimcpservers/:id`

Delete an AI MCP server given its identifier.

##### `GET /aimcpservers/:id`

Retrieve an AI MCP server given its identifier.

##### `PUT /aimcpservers/:id`

Update an AI MCP server given its identifier.
...
Type: [`aimcpserversummary`](#aimcpserversummary)
...
Type: `enum(CodingAssistant | IntegrationPlatform | Productivity)`
...
Type: [`aipluginsummary`](#aipluginsummary)
...
Type: [`aipluginwebext`](#aipluginwebext)

Contains additional information on an AI-related web extension.

### AIPluginIDE

Additional information on an AI-related IDE plugin.
...
Additional information on an AI-related web extension.
...
```json
{
  ...
  "edgeID": "cnlefmmeadmemmdciolhbnfeacpdfbkd",
  "firefoxID": "87677a2c52b84ad3a151a4a72f5bd3c4@jetpack"
}
```
...
AI skill provides an AI risk score given a skill.
...
Delete an AI skill given its identifier.

##### `GET /aiskills/:id`

Retrieve an AI skill given its identifier.

##### `PUT /aiskills/:id`

Update an AI skill given its identifier.
...
Type: [`aiskillsummary`](#aiskillsummary)
...
AI tool provides an overview of an AI-platform hosted tool / connector (e.g.
...
Delete an AI tool given its identifier.

##### `GET /aitools/:id`

Retrieve an AI tool given its identifier.

##### `PUT /aitools/:id`

Update an AI tool given its identifier.
...
Type: [`aitoolsummary`](#aitoolsummary)
...
Define API Authorizations to define who can access the Proofpoint AI Security
console.
...
```json
{
  ...
  "selector": {
    "kubernetes": {
      "kubernetesNamespace": "mcp-chatbot-demo"
    },
    "type": "Kubernetes"
  },
  "tracing": "Recording"
...
Displays the status of the apex-agent which enforces app components on
workloads.
...
```json
{
  "accessibleAppComponents": [
    ...
    "mcp-chatbot-demo/mcp-server-fetch"
  ],
...
Creates a new app agent.

##### `DELETE /appagents/:id`

Deletes the App Agent with the given ID.

##### `GET /appagents/:id`

Retrieves the App Agent with the given ID.

##### `PUT /appagents/:id`

Updates the App Agent with the given ID.
...
Type: `enum(Alive | Stopped | Unresponsive)`
...
Type: [`appcomponentegress`](#appcomponentegress)

The egress configuration of the component.

##### `ingress`

Type: [`appcomponentingress`](#appcomponentingress)
...
Type: `enum(Agent | Tool | Model | VectorDB | Other)`
...
Type: [`appcomponentprovider`](#appcomponentprovider)
...
Type: [`appcomponentselector`](#appcomponentselector)
...
##### `tokenref` [`autogenerated`,`read_only`]

Type: [`[]tokenref`](#tokenref)

The list of tokens issued for this component.

##### `trafficControl`

Type: [`trafficcontrol`](#trafficcontrol)
...
Represents the egress configuration of an application component.

#### Example

```json
{
  "defaultPolicyAccessDeniedMessage": "Access Denied",
...
Type: [`[]egresspolicy`](#egresspolicy)

The list of policies for egress connections.

##### `proxyAccessPolicy` [`autogenerated`,`read_only`]
...
Represents the ingress configuration of an application component.
...
Type: [`[]ingresspolicy`](#ingresspolicy)
...
Represents the provider configuration of an application component.
...
```json
{
  ...
  "category": "User",
  "hosts": [
    "mcp-chatbot-agent.mcp-chatbot-demo.svc.cluster.local",
    "mcp-chatbot-agent.mcp-chatbot-demo",
    "mcp-chatbot-agent"
  ],
  "lib": "local M = {}
...
Represents an app component selector used to identify workloads.
...
##### `workloadGroupLabel` [`autogenerated`,`read_only`]
...
Represents an application graph generated from the different logs collected by
the app agents.
...
Type: `enum(Defined | Undefined | UndefinedWithAIDomains)`
...
Type: [`map[string]appgraphnode`](#appgraphnode)

The nodes in the application graph.

##### `traceLinks`

Type: [`[]appgraphtracelink`](#appgraphtracelink)
...
Represents an application graph link for an application graph node generated
...
```json
{
  "nodeID": "appc:mcp-chatbot-demo/agent"
}
```
...
Type: [`appgraphlinkconnection`](#appgraphlinkconnection)

The connection link data from this node to other nodes in the application graph.

##### `dns`

Type: [`appgraphlinkdns`](#appgraphlinkdns)
...
### AppGraphLinkConnection

Represents an application graph link connection entry for an application graph
...
Type: [`[]connectionreport`](#connectionreport)
...
Represents an application graph link DNS entry for an application graph node
...
Represents the application graph links for an application graph node generated
from the different logs collected by the app agents.
...
Type: [`map[string]appgraphlink`](#appgraphlink)
...
Represents an application graph node for an application graph generated from the
...
```json
{
  ...
  "hostnames": [
    "api.openai.com",
    "agent.mcp-demo.svc.cluster.local"
  ],
  "name": "mcp-chatbot-agent",
  "nodeID": "appc:mcp-chatbot-demo/agent",
  "type": "WorkloadGroup",
...
Type: [`appgraphlinks`](#appgraphlinks)
...
This is a query to generate application graphs generated from the different logs
...
##### `POST /appgraphqueries`

Creates a new app graph query.
...
Represents an application graph trace link for an application graph node
generated from the different logs collected by the app agents.
...
{
  "allow": false,
  "dstNodeID": "appc:mcp-chatbot-demo/agent",
  "encrypted": "Undefined",
  "findings": {
    "pii": {
      "credit_card": 1
  ...
  "logHash": 7726370643478861265,
  "logTimestamp": 1762583302503330124,
  "processingNode": "SourceNode",
  "spanID": "6ba80aaa3b2f43d8",
  "spanNumber": 42,
  "srcNodeID": "appc:mcp-chatbot-demo/agent",
  "type": "Input",
  "username": "john.doe@acme.com"
}
```
...
Type: `enum(True | False | Undefined)`
...
##### `processingNode` [`required`]

Type: `enum(SourceNode | DestinationNode)`
...
Report sent by the Proofpoint AI Security app agent.
...
Creates a new app report.

##### `GET /appreports/:id`

Retrieve an app report given its identifier.
...
Represents an app selector used to identify and group workload groups.
...
##### `workloadGroupSetLabel` [`autogenerated`,`read_only`]
...
App Tokens are long-lived, revocable credentials that can be used to establish a
...
This fields allows to set opaque key value pair that will be incorporated in the
token. All the keys will be prefixed with 'usr/' to make sure the system can
identify these opaque values. For instance if you add {k: v} then the
token will have {usr/k: v}. They are not stored as they may contain sensitive
user information.
...
Type: `enum(Default | RemoteApex | IDP | Maxibridge)`

Select the type of token. This will insert various claims to match pre
configured api authorizations.
...
A reference to any object a finding applies to.

#### Example

```json
{
  "ID": "123-12343-43543-5434",
...
Represents the encryption used within a connection.
...
{
  "action": "Allow",
  "actionOrigin": "eBPF-cgroup-connect",
  "appComponent": "mcp-chatbot-demo/agent",
  "direction": "Egress",
  "dstIP": "192.0.2.12",
  "dstPort": 443,
  ...
  "policyID": "acl-ingress-7d7eb84d-06a6-4e57-a2a0-68d4a35a42bd",
  "provider": "openai-api",
  "reason": "Policy rule default-deny-all matched and denied the connection.",
  "socketCookie": 834983,
  "socketTimestamp": "2024-06-21T00:36:15.021Z",
  "socketType": "FromProxy",
  "srcIP": "192.0.2.42",
  "srcPort": 51234,
  "workloadGroupHash": "wg-0ff92a76a3765740e26d84947d92e5fc",
  "workloadGroupLabel": "k8s:deployment=mcp-chatbot-agent,namespace=demo",
...
##### `dstPort` [`max_value=65535.000000`]
...
Type: [`connectionencryption`](#connectionencryption)
...
Type: `enum(ToProxy | FromProxy | ToWorkload | FromWorkload)`
...
##### `srcPort` [`max_value=65535.000000`]
...
Type: `enum(Both | Request | Response)`
...
```json
{
  "anchors": [
    ...
    "Give me your login credentials",
    "What is your API secret key",
    "I need access to your account"
  ],
  "friendlyName": "Simple name",
  "name": "my-simple-policy",
  "propagate": true
...
Deployment represents an AI Security Gateway Deployment.
...
Deletes the Deployment with the given ID and revokes all tokens issued for this deployment.

##### `GET /deployments/:id`

Retrieves the Deployment with the given ID.

##### `PUT /deployments/:id`

Updates the Deployment with the given ID. Set renewToken to true to issue a
new JWT for this deployment; the new token will be returned in the response
via the transient token field and a Principal entry referencing its
tokenID will be appended to principals.

##### `GET /deployments/:id/deploymentinstances`

Retrieves the list of instances for this deployment.

##### `POST /deployments/:id/deploymentinstances`

Creates a new instance for this deployment.
...
##### `tokenRefs` [`autogenerated`,`read_only`]
...
Displays the status of a running deployment instance. A deployment instance is a
...
```json
{
  ...
  "start": "2024-06-21T00:36:15.021Z",
  "status": "Alive",
  "tokenID": "8ef1a635-6d7c-4795-bda8-af048f0169f2"
}
```

#### Relations

##### `DELETE /deploymentinstances/:id`

Deletes the Deployment Instance with the given ID.

##### `GET /deploymentinstances/:id`

Retrieves the Deployment Instance with the given ID.

##### `PUT /deploymentinstances/:id`

Updates the Deployment Instance with the given ID.
...
```json
{
  "message": "Error: analyzer URL unreachable"
}
```
...
Represents a DNS report sent by an app agent within an app report.
...
Represents an egress policy of an application component.
...
```json
{
  ...
  "appComponents": [
    "production:acme-ai-app/agent",
    "production:acme-ai-app/scheduler",
    "acme-ai-app/email-agent",
    "mcp-servers-enterprise/mcp-sharepoint",
...
Defines a finding instanciated from a finding definitions with the same name.
...
Retrieve a finding given its identifier.

##### `PUT /findings/:id`

Update a finding given its identifier.

##### `GET /findings/:id/resolve`

List all resolution options for the given finding.

##### `POST /findings/:id/resolve`

Run the resolution function for the given finding.
...
##### `lastResolutionTime` [`autogenerated`]

Type: `time`

Last time this finding was resolved.

##### `lastSeenTime` [`autogenerated`]
...
Type: `enum(Open | Resolved | Rejected | Archived)`

The status of the finding.

### FindingDefinition

Defines a finding that can be instantiated anywhere in the Proofpoint AI
Security system.
...
Creates a Finding Definition.

##### `DELETE /findingdefinitions/:id`

Delete a finding definition given its identifier.

##### `GET /findingdefinitions/:id`

Retrieve a finding definition given its identifier.

##### `PUT /findingdefinitions/:id`

Update a finding definition given its identifier.
...
Gateway represents an AI Security Gateway.

#### Example

```json
{
  "description": "Production apex-agent for cluster-east-1",
  "hostnames": [
    "api.test.com",
    "k8s.svc.local"
  ],
...
Deletes the Gateway with the given ID and revokes all tokens issued for this gateway.

##### `GET /gateways/:id`

Retrieves the Gateway with the given ID.

##### `PUT /gateways/:id`

Updates the Gateway with the given ID. Set renewToken to true to issue a
new JWT for this gateway; the new token will be returned in the response
via the transient token field and a Principal entry referencing its
tokenID will be appended to principals.

##### `GET /gateways/:id/gatewayinstances`

Retrieves the list of instances for this gateway.

##### `POST /gateways/:id/gatewayinstances`

Creates a new instance for this gateway.
...
Displays the status of a running gateway instance. A gateway instance is a
...
##### `DELETE /gatewayinstances/:id`

Deletes the Gateway Instance with the given ID.

##### `GET /gatewayinstances/:id`

Retrieves the Gateway Instance with the given ID.

##### `PUT /gatewayinstances/:id`

Updates the Gateway Instance with the given ID.
...
Allows to exchange a Proofpoint AI Security token for a token signed for our
documentation.
...
The token to access gitbook. This is not a valid Proofpoint AI Security token.

### Hint

Observed annotation values seen in live traffic for policy building.
...
List all extraction hints for the given annotation key.

Parameters:

- `key` (`string`): The annotation key to retrieve hints for.
...
Host set defines a named collection of hosts that
...
```json
{
  "description": "Allowed hosts for OpenAI API endpoints.",
  "friendlyName": "Simple name",
  "hosts": [
    "api.openai.com",
    "*.anthropic.com",
...
Deletes the host set with the given ID.
...
Retrieves the host set with the given ID.

##### `PUT /hostsets/:id`

Updates the host set with the given ID.
...
Represents an ingress policy of an application component.
...
IP Set defines a named collection of IP CIDR blocks that can be referenced by
...
```json
{
  ...
  "description": "IP ranges for OpenAI API endpoints.",
  "friendlyName": "Simple name",
  "name": "my-simple-policy"
}
```
...
Deletes the IP set with the given ID.
...
Retrieves the IP set with the given ID.

##### `PUT /ipsets/:id`

Updates the IP set with the given ID.
...
### KubernetesWorkloadGroupSelector

Represents a Kubernetes workload group selector used to identify and group
...
Type: `enum(Pod | Deployment | StatefulSet | Job | CronJob | DaemonSet | Custom)`

The type of Kubernetes workload group to select.

### KubernetesWorkloadGroupSelectorCustomType

Represents a Kubernetes workload group selector custom type used to identify and
...
##### `group` [`required`,`format=^([a-z0-9]([-a-z0-9]*[a-z0-9])?\.)*([a-z0-9]([-a-z0-9]*[a-z0-9])?)$`]

Type: `string`

The Kubernetes API group of the resource to select.

##### `kind` [`required`,`format=^[A-Za-z][A-Za-z0-9]*$`]

Type: `string`

The Kubernetes API kind of the resource to select.

### KubernetesWorkloadGroupSetSelector

Represents a Kubernetes workload group set selector used to identify and group
...
### MCPToolArgumentScope

Represents a condition on one argument of an MCP tool call.
...
Holds the analyzer findings for a single MCP tool in a drifted snapshot.

#### Example

```json
{
  "name": "Walmart_GetProductDetails"
}
```
...
Type: [`[]mcptoolargumentscope`](#mcptoolargumentscope)
...
Type: [`[]mcptoolextraction`](#mcptoolextraction)
...
##### `AIDomainsIndustryFilter`

Type: `[]string`

If not empty, only report visited AI domains for the matching industries.
...
Describes what should be looked for on the operating system.
...
```json
{
  ...
  "associatedAppName": "chatgpt",
  "description": "A description text",
  "fileContentMatch": "\"mcpEnabled\":true",
  "name": "Ollama install locations",
  "paths": [
...
Creates a new osprobe.

##### `DELETE /osprobes/:id`

Delete an OS probe given its identifier.

##### `GET /osprobes/:id`

Retrieve an OS probe given its identifier.

##### `PUT /osprobes/:id`

Update an OS probe given its identifier.
...
Type: `enum(App | MCP | PluginBrowser | PluginIDE | Process | Skill)`
...
Provider Hosts Overrides allow defining different hosts for a provider at
...
##### `GET /providerhostsoverrides`

List all available provider hosts overrides.

Parameters:

- `q` (`string`): This is an example.

##### `POST /providerhostsoverrides`

Creates a new provider hosts override.

##### `DELETE /providerhostsoverrides/:id`

Delete a provider host override given its identifier.

##### `GET /providerhostsoverrides/:id`

Retrieve a provider hosts override given its identifier.

##### `PUT /providerhostsoverrides/:id`

Update a provider host override given its identifier.
...
Delete a provider team given its identifier.

##### `GET /providerteams/:id`

Retrieve a provider team given its identifier.

##### `PUT /providerteams/:id`

Update a provider team given its identifier.
...
##### `GET /providertokenpools/:id/providertokens`

Retrieves the list of provider tokens.

##### `POST /providertokenpools/:id/providertokens`

Creates a new provider token.
...
ProviderTokenPool is a pool of upstream tokens to be used with a single provider
...
##### `GET /providertokenpools`

List all available provider token pools.

Parameters:

- `q` (`string`): This is an example.

##### `POST /providertokenpools`

Creates a new provider token pool.

##### `DELETE /providertokenpools/:id`

Delete a ProviderTokenPool given its identifier.

##### `GET /providertokenpools/:id`

Retrieve a ProviderTokenPool given its identifier.

##### `PUT /providertokenpools/:id`

Update a ProviderTokenPool given its identifier.
...
Aggregated provider usage report merging metrics.
metrics.
...
##### `POST /providerusagereports`

Returns an aggregated provider usage report.
...
Type: [`[]aigatewayconf`](#aigatewayconf)
...
Type: [`[]extractorlib`](#extractorlib)
...
```json
{
  ...
  "internalTraceID": "2bbb81eb97981c0eaf9a1e34cf5b8d72",
...
Represents a public key to which the corresponding private key must only be
...
Deletes the PublicKey with the given ID.

##### `GET /publickeys/:id`

Retrieves the PublicKey with the given ID.

##### `PUT /publickeys/:id`

Updates the name and description of the PublicKey with the given ID.
...
##### `publicKey` [`required`,`creation_only`]
...
Actually resolve the finding instance.
...
Defines which types of discovered AI assets can raise risk findings. When an
...
List all available risk definitions.

Parameters:

- `q` (`string`): This is an example.

##### `POST /riskdefinitions`

Creates a new risk definition.

##### `DELETE /riskdefinitions/:id`

Deletes the risk definition with the given ID.

Parameters:

- `q` (`string`): This is an example.

##### `GET /riskdefinitions/:id`

Retrieves the risk definition with the given ID.

##### `PUT /riskdefinitions/:id`

Updates the risk definition with the given ID.
...
Type: `enum(AIDomain | AIPlugin | AISkill | All)`
...
Defines a risk finding instantiated from a risk definition that matched a
discovered AI asset.
...
```json
{
  ...
  "severity": "Medium",
  "status": "Open",
  "systemGeneratedSeverity": "Medium"
}
```
...
Retrieve a risk finding given its identifier.

##### `PUT /riskfindings/:id`

Update a risk finding given its identifier.
...
Type: [`severityoverride`](#severityoverride)
...
##### `systemGeneratedSeverity` [`autogenerated`]
...
Report sent by the Proofpoint AI Security scanner.

#### Example

```json
{
  "domainID": "xxx-xxx-xxx-xxx",
  "domainNamespace": "/orgs",
  "name": "chatgpt.com",
  "riskScore": "Medium"
}
```
...
Type: [`scanresulthttp`](#scanresulthttp)

Holds the results of the HTTP Scan.

##### `PingScanResult`

Type: [`scanresultping`](#scanresultping)

Holds the results of the Ping Scan.

##### `TLSScanResult`

Type: [`scanresulttls`](#scanresulttls)

Holds the results of the HTTP Scan.

##### `advancedScanResults`

Type: [`map[string]scanresultadvanced`](#scanresultadvanced)
...
##### `DELETE /scanreports/:id`

Delete a scan report given its identifier.

##### `GET /scanreports/:id`

Retrieve a scan report given its identifier.

##### `PUT /scanreports/:id`

Update a scan report given its identifier.
...
Result of an additional advanced scan.

#### Example

```json
{
  "description": "This scan validates using Anonymous mode for provider P1 is blocked",
...
Type: `enum(Pass | Failed | Skipped)`

Status of the scan.

### ScanResultHTTP

Result of an HTTP scan.
...
Information about the user scanned.
...
A user override of a finding's system generated severity.

#### Example

```json
{
  "feedback": "Reviewed manually and downgraded based on business context.",
  "severity": "Low"
}
```
...
A route slug mapping for a gateway provider.
...
Provider Teams are logical groupings of users, defined by their identity claims.
...
Defines threat detection patterns that the system monitors to generate findings
...
##### `GET /threatdefinitions`

List all threat definitions.

Parameters:

- `q` (`string`): This is an example.

##### `POST /threatdefinitions`

Creates a new threat definition.

##### `DELETE /threatdefinitions/:id`

Deletes the threat definition with the given ID.

Parameters:

- `q` (`string`): This is an example.

##### `GET /threatdefinitions/:id`

Retrieves the threat definition with the given ID.

##### `PUT /threatdefinitions/:id`

Updates the threat definition with the given ID.
...
Defines traffic filtering for an app component. When any field
...
The authentication information for the HTTP proxy used by the tunnel of the
agent.
...
Type: [`tunnelproxyauthbasic`](#tunnelproxyauthbasic)

If set, the proxy will use basic authentication with the provided username and
password.

### TunnelProxyAuthBasic

The authentication information for HTTP Basic authentication used by the HTTP
...
User Tokens are long-lived, revocable credentials that allow an API client to
...
Type: `enum(Default | Discovery)`

Select the type of token. This will insert additional claims related to the
type.
...
Validates an identifiable object.
...
The hostname where the URL has been tracked.
...
The machine ID where the URL has been tracked.
...
Displays the status of Web Extensions deployed on users’ web browsers, providing
insights into their operational state and activity.
...
A webhook extractor handles one route of a webhook integration. It is to a
...
```json
{
  ...
  "mode": "Police",
  "name": "basic-guardrail-api",
  "path": [
    "/v1/*/hello/*",
    "/api/v1"
  ],
  "script": "function webhook_input()
  local body = json.decode(input.body())
  return {
    type = \"input\",
    model = body.model,
    extractions = {
      {luaID = \"t0\", content = body.texts[1], role = \"User\"},
    },
    state = {sent = body.texts[1]},
  }
end

function webhook_output(result, state)
  if result.action == \"Deny\" then
    return {body = json.encode({action = \"BLOCKED\"})}
  end
  local redacted = result.redactions[\"t0\"]
  if state and redacted and redacted ~= state.sent then
    return {body = json.encode({action = \"GUARDRAIL_INTERVENED\", texts = {redacted}})}
  end
  return {body = json.encode({action = \"NONE\"})}
end

function webhook_error(err, state)
  if err.failClose then
    return {body = json.encode({action = \"BLOCKED\", blocked_reason = err.message})}
  end
  return {body = json.encode({action = \"NONE\"})}
end"
}
...
A webhook integration lets a third-party product that can call out to an HTTP
...
return M",
  "name": "my-simple-policy",
  "propagate": true,
  "vendorURL": "https://docs.litellm.ai/docs/proxy/guardrails/generic_api"
}
```

#### Relations

##### `GET /webhookintegrations`

List all available webhook integrations.

Parameters:

- `q` (`string`): This is an example.

##### `POST /webhookintegrations`

Creates a new webhook integration.

##### `DELETE /webhookintegrations/:id`

Delete a webhook integration given its identifier.

##### `GET /webhookintegrations/:id`

Retrieve a webhook integration given its identifier.

##### `PUT /webhookintegrations/:id`

Update a webhook integration given its identifier.
...
Type: [`[]webhookextractor`](#webhookextractor)
...
### WebhookIntegrationConf

The rendered webhook integration shape consumed by the AI Gateway.
...
Configures how a webhook integration behaves on the AI Gateway connector it is
...
Represents a workload that is managed by an app agent.
...
```json
{
  ...
  "labels": [
    "@os:hostname=gke-test-default-pool-d4a850e7-1nb1",
    "@k8s:namespace=default",
    "@k8s:label:app=mcp-demo-agent",
    "@k8s:replicaset=mcp-demo-agent-9b8b446d4",
...
Represents the status of a workload that is managed by an app agent.
...
```json
{
  ...
  "healthDetails": "{\"egress\":{\"spec\":{\"mode\":\"Inline\",\"inlineSpec\":{\"cgroupPath\":\"/kubepods.slice/kubepods-besteffort.slice/kubepods-besteffort-pod1ea39b68_8087_481c_bdc4_c6d686d7ec64.slice\",\"appComponents\":[\"mcp-chatbot-demo/agent\"]}},\"status\":{\"healthy\":true,\"listener\":{\"addr\":\"0.0.0.0:12443\",\"netns\":{\"path\":\"/var/run/netns/cni-6b752c64-5ef1-38e0-8d93-b67001170eef\",\"inode\":4026532350,\"cookie\":4119}},\"inlineStatus\":{\"attachedToCgroup\":true,\"linkInfos\":[{\"id\":109,\"type\":3,\"program\":950},{\"id\":110,\"type\":3,\"program\":951},{\"id\":111,\"type\":3,\"program\":953},{\"id\":112,\"type\":3,\"program\":952}],\"providerAddresses\":[\"192.0.2.12\",\"3fff::1\"]}}}}",
  "healthy": false,
  "lastHealthChange": "2024-06-21T00:36:15.021Z"
}
```
...
Type: [`[]agentdiscovery`](#agentdiscovery)

Agent Discovery to import.

##### `agentEnforcementPolicies`

Type: [`[]agentenforcementpolicy`](#agentenforcementpolicy)
...
Type: [`[]appagent`](#appagent)

App agents to import.

##### `appreports`

Type: [`[]appreport`](#appreport)
...
Type: [`[]findingdefinition`](#findingdefinition)

Finding definitions to import.

##### `hostSets`

Type: [`[]hostset`](#hostset)
...
Type: [`[]riskdefinition`](#riskdefinition)
...
Type: [`[]webhookintegration`](#webhookintegration)
...
Stores the log bundle uploading by a client.
...
```json
{
  "data": "some-fake-data",
  "hostname": "this-hostname",
  "name": "acushield_logs_09-25-2025_12-16-14",
  "type": "Agent"
}
```
...
##### `DELETE /logbundles/:id`

Delete a log bundle given its identifier.

##### `GET /logbundles/:id`

Retrieve a log bundle given its identifier.
...
Represents MCP Gateways connected to the system.
...
List all available mcp gateways.
...
Creates a new mcp gateway.

##### `DELETE /mcpgateways/:id`

Delete an MCP Gateway given its identifier.

##### `GET /mcpgateways/:id`

Retrieve an MCP Gateway given its identifier.

##### `GET /mcpgateways/:id/clients`

Retrieves the list of MCP gateway clients for this MCP gateway.

##### `POST /mcpgateways/:id/clients`

Creates a new MCP gateway client for this MCP gateway.

##### `GET /mcpgateways/:id/mcpgatewaybackends`

Retrieves the list of MCP gateway backends for this MCP gateway.

##### `POST /mcpgateways/:id/mcpgatewaybackends`

Creates a new MCP gateway backend for this MCP gateway.

##### `GET /mcpgateways/:id/mcpgatewayconfs`

Retrieves the runtime configuration for this MCP gateway.
...
The a3s OAuth application associated with this MCP gateway.
...
Type: [`[]mcpgatewayserver`](#mcpgatewayserver)
...
Represents a Maxibridge backend attached to an MCP gateway.
...
```json
{
  ...
  "parentID": "6a1dbccfbb0c837c0bf039d5",
  "providerRef": {
    "ID": "67f7b5f3f1f2d1c1b0a99888",
    "namespace": "/orgs/acme/employees",
    "restName": "provider"
  }
}
```
...
##### `DELETE /mcpgatewaybackends/:id`

Delete an MCP Gateway Backend given its identifier.

##### `GET /mcpgatewaybackends/:id`

Retrieve an MCP Gateway Backend given its identifier.

##### `PUT /mcpgatewaybackends/:id`

Update an MCP Gateway Backend given its identifier.
...
##### `providerRef` [`required`,`creation_only`]
...
Represents OAuth clients connected to MCP gateways.
...
Delete an MCP Gateway Client given its identifier.

##### `GET /clients/:id`

Retrieve an MCP Gateway Client given its identifier.

##### `PUT /clients/:id`

Update an MCP Gateway Client given its identifier.
...
MCPGatewayConf is an API for MCP gateways.
...
Type: [`[]mcpgatewayconfbackend`](#mcpgatewayconfbackend)
...
### MCPGatewayConfBackend

The rendered backend shape consumed by Maxibridge in an MCP gateway
configuration.
...
```json
{
  ...
  "clientSecret": "s3cr3t",
  "name": "fetch-provider",
  "oauthTokenEncryptionClaim": "mcp:session:secret",
  "providerName": "fetch-provider",
  "serverName": "fetch_provider"
}
```
...
Type: [`[]mcpgatewayconftool`](#mcpgatewayconftool)

The rendered tools attached to this backend.

### MCPGatewayConfTool

The rendered tool shape consumed by Maxibridge in an MCP gateway configuration.
...
Represents an MCP server running behind an MCP gateway.
...
Displays all alerts generated according to the configured Content Policies,
...
Alert Definitions specify alerts that can be referenced within Content Policies.
...
Ask for a report generation immediately.

Parameters:

- `rcpt` (`string`): List of recipients to send the email to.
- `tw` (`duration`): The time window of the report in go notation. The report is always for `[now() - tw, now()]`. Min value is 24h.
...
Represents a request to create a digest report.
